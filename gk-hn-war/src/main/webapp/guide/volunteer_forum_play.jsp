<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>志愿讲堂播放页面</title>
    <link rel="stylesheet" href="http://cdn.gaokao360.net/static/global/guide/styles/volunteer_forum_play.min.css" />
</head>
<body>
<div class="header">
    <div class="w1000">
        <a href="">
            <img src="http://cdn.gaokao360.net/static/global/common/images/logo-min.png" alt="高考360" class="logo fl"/>
        </a>
        <ul class="main-menu fl">
            <li><a href="">首页</a></li>
            <li><a href="">报考指南</a></li>
            <li><a href="">报考咨询</a></li>
            <li><a href="">考前备考</a></li>
            <li><a href="">考后报考</a></li>
        </ul>
        <div class="user-info-list fr">
            <div class="user">
                <img src="http://cdn.gaokao360.net/static/global/common/images/avatar.png" alt="avatar" class="user-avatar"/>
                <a href="">韩小寒</a>
            </div>
            <ul class="menu hide">
                <li><a href="">个人信息</a></li>
                <li><a href="">修改密码</a></li>
                <li><a href="">退出</a></li>
            </ul>
        </div>
    </div>
</div>

<div class="content">
    <div class="banner">
        <video id="home_video" controls="" preload="none" poster="http://www.videojs.com/img/poster.jpg" class="video-js vjs-default-skin">
            <source src="http://vjs.zencdn.net/v/oceans.mp4" type="video/mp4">
            <source src="http://vjs.zencdn.net/v/oceans.webm" type="video/webm">
            <track kind="captions" src="/vtt/captions.vtt" srclang="en" label="English">
            </track>
        </video>
    </div>
    <div class="mian-body w1000">
        <h1>徐老师<span>语文</span></h1>
        <h6>简介：</h6>
        <p>徐老师，正高级教师，任教于西北工业大学附属中学，高中语文教研组长。西安市教育学会特聘研究员。湖南省教学能手， 湖南省首批学科带头人。湖南省中语会理事。湖南师范大学硕士合作指导教师。先后有15年参加湖南省高考语文评卷工作，曾担任题小组长、题组长。</p>
        <h1>相关视频 <a>more</a></h1>
        <div class="episode-num mt20">
            <a class="active">第一集</a>
            <a>第二集</a>
            <a>第一集</a>
            <a>第一集</a>
            <a>第一集</a>
            <a>第二集</a>
            <a>第一集</a>
            <a>第一集</a>
            <a>第一集</a>
            <a>第二集</a>
            <a>第一集</a>
            <a>第一集</a>
            <a>第一集</a>
        </div>
    </div>
</div>

<div class="footer">
    <div class="bottom-nav">
        <div class="w1000">
            <ul>
                <li><a href="" class="nav-title">关于高考360</a></li>
                <li><a href="">联系我们</a></li>
                <li><a href="">使用协议</a></li>
            </ul>
            <ul>
                <li><a href="" class="nav-title">帮助中心</a></li>
                <li><a href="">常见问题</a></li>
                <li><a href="">意见反馈</a></li>
            </ul>
            <ul>
                <li><a href="" class="nav-title">关注我们</a></li>
                <li><a href="">官方微信</a></li>
                <li><a href="">新浪微博</a></li>
            </ul>
            <img src="http://cdn.gaokao360.net/static/global/common/images/logo.png" alt="logo" class="logo"/>
        </div>
    </div>
    <div class="copy-right">
        <p class="w1000">联系信息:E-mail:cewfefe@163.com QQ:5454555 备案号:陕ICP备343454135-1号 [站长统计]</p>
    </div>
</div>
</body>
</html>
